...
def organize_data(data, engines, mappings, metric):
    ...
    for i in range(0, len(data)):
        if data.iloc[i][metric] == "OutOfMemory":
            average_data.loc[len(average_data)] = [data.iloc[i]['engine'], data.iloc[i]['mapping'], math.log10(MAX_MEMORY)]
        else:
...
def plot(data, scale):
    ...
    ordered_data = organize_data(data, [x.lower() for x in engines], mappings, 'memory_max')
    ...
    ticks.append('OutOfMem')
    plt.yticks(np.arange(0, len(ticks)), ticks, fontsize=16)
    plt.ylim(top=min(math.log10(10**(max_value)), math.log10(MAX_MEMORY)))
    ylabel = "Maximum memory (log$_{10}$(kB))"
    ...
    filename = "./figures/mem_" + scale

    plt.savefig(filename + ".png", bbox_inches='tight', dpi=700)

    plt.clf()

    print("Finished: 'figures/mem_" + scale +"'")
