...
def plot(data, scale, overhead = False):
    ...
    plt.bar(r1, ordered_data.values.tolist()[0], width=barWidth, color='#A46593',#A46593
                label='morph-kgc')
    plt.bar(r2, ordered_data.values.tolist()[1], width=barWidth, color='#2862CC',#2862CC
                label='morph-kgc-p')
    plt.bar(r3, ordered_data.values.tolist()[2], width=barWidth, color='#90AFE9',#90AFE9
                label='mapping-template')
    plt.bar(r4, ordered_data.values.tolist()[3], width=barWidth, color='#A5D9A5',#A5D9A5
                label='mapping-template-nj')


    plt.xticks([r + barWidth*2.5  for r in range(len(r1))], mappings, fontsize=16)

    plt.yticks(np.arange(0, 6), ('0.0', '1.0', '2.0', '3.0', '4.0', 'Timeout'), fontsize=16)
    plt.ylim(top=math.log10(100000))
